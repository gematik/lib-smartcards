...
project.version = "0.6.6" // for history of releases see CHANGELOG.md

group = "de.gematik.smartcards"

val libraryDescription = "Cryptographic objects"
...
tasks.withType<GenerateMavenPom>().configureEach {
  doFirst {
    ...
    pom.name.set("Cryptographic objects")
...
dependencies {
  ...
  testImplementation(libs.bcprov.jdk18on) // bouncy castle
...
tasks.withType(Test::class) {
  ...
  // de.gematik.smartcards.crypto:puToMaLo".
  // Note 3: The execution time of the following test is non-deterministic:
  //         TestPrivateKeyCrtKeyImpl.test_gakpLog2__int_BigInteger()
  //         Be aware of that when comparing performance.

  // no statement about parallel test execution                      arthur => 1' 27" .. 1' 32"
  // maxParallelForks = 1                                         // arthur => 1' 20" .. 1' 40"
  // maxParallelForks = 4                                         // arthur =>    39" ..    40"
  // maxParallelForks = 8                                         // arthur =>    47" ..    54"
  // maxParallelForks = 16                                        // arthur =>    42" ..    44"
  val noProcessors = Runtime.getRuntime().availableProcessors()
  maxParallelForks = (noProcessors - 1).takeIf { it > 0 } ?: 1 // arthur =>    41" ..    58"
}
